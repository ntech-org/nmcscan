...
impl MinecraftAccountRepository {
    ...
    pub async fn add_account(&self, email: String, password: Option<String>, access_token: Option<String>, refresh_token: Option<String>, expires_at: Option<chrono::NaiveDateTime>) -> Result<minecraft_accounts::Model, DbErr> {
        ...
        minecraft_accounts::Entity::insert(model).exec_with_returning(&self.db).await
    }
    ...
    pub async fn delete_account(&self, id: i32) -> Result<DeleteResult, DbErr> {
        minecraft_accounts::Entity::delete_by_id(id).exec(&self.db).await
    }

...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        db.execute_unprepared(
            "ALTER TABLE servers ADD COLUMN IF NOT EXISTS flags TEXT DEFAULT ''"
        ).await?;
        ...
        db.execute_unprepared("DROP TRIGGER IF EXISTS trg_compute_server_flags ON servers").await?;
        db.execute_unprepared(r#"
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        db.execute_unprepared("DROP TRIGGER IF EXISTS trg_compute_server_flags ON servers").await?;
        db.execute_unprepared("DROP FUNCTION IF EXISTS compute_server_flags()").await?;
        db.execute_unprepared("DROP INDEX IF EXISTS idx_servers_flags").await?;
        db.execute_unprepared("ALTER TABLE servers DROP COLUMN IF EXISTS flags").await?;
        db.execute_unprepared("ALTER TABLE servers DROP COLUMN IF EXISTS last_login_at").await?;
        db.execute_unprepared("ALTER TABLE servers DROP COLUMN IF EXISTS login_obstacle").await?;

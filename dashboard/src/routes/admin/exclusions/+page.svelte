<script lang="ts">
  ...
  async function loadExclusions() {
    loading = true;
    try {
      const res = await fetchWithAuth(`/api/exclude?page=${page}&limit=${limit}`);
...
<div class="space-y-6">
  <div class="flex items-center justify-between">
    <div class="space-y-1">
      ...
      <p class="text-muted-foreground italic text-sm">Restrict scanner access to specific networks or critical infrastructure.</p>
    </div>
  ...
  <Card.Root class="bg-card shadow-lg border-muted overflow-hidden">
    <Card.Header class="p-6">
      ...
      <Card.Description class="italic">The scanner will immediately stop targeting these ranges.</Card.Description>
      <form onsubmit={(e) => { e.preventDefault(); addExclusion(); }} class="flex flex-col md:flex-row gap-4 mt-4">
        <div class="flex-1">
          <Input 
    ...
    <Card.Content class="p-0 border-t">
      <Table.Root>
        ...
        <Table.Body>
          {#if loading}
            <Table.Row>
              <Table.Cell colspan={4} class="h-24 text-center italic text-muted-foreground">
          ...
          {:else}
            {#each exclusions as entry}
              <Table.Row class="group italic">
                <Table.Cell class="font-mono text-sm font-semibold text-destructive">
                  {entry.network}
                </Table.Cell>
                <Table.Cell>
                  <Badge variant="destructive" class="text-[9px] px-1.5 py-0 uppercase font-bold tracking-tighter">
                    BLOCKED
                  </Badge>
                </Table.Cell>
                <Table.Cell class="text-sm text-muted-foreground truncate max-w-md">
                  {entry.comment || 'No reason provided'}
                </Table.Cell>
                <Table.Cell class="text-right pr-6">
                  <Button variant="ghost" size="icon" class="h-8 w-8 rounded-full text-muted-foreground/50 hover:text-destructive transition-all">

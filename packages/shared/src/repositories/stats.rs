...
impl StatsRepository {
    ...
    pub async fn refresh_materialized_views(&self) -> Result<(), DbErr> {
        self.db
            .execute_unprepared("REFRESH MATERIALIZED VIEW CONCURRENTLY asn_stats")
            .await?;
        self.db
            .execute_unprepared("REFRESH MATERIALIZED VIEW CONCURRENTLY global_stats")
            .await?;
        Ok(())
    }
    ...
    pub async fn increment_batch_stats(
        &self,
        hot: i32,
        warm: i32,
        cold: i32,
        ...
        if hot == 0 && warm == 0 && cold == 0 && discoveries == 0 {
            return Ok(());
        }
        let date = Utc::now().date_naive();
    ...
    pub async fn get_global_stats(&self) -> Result<(i64, i64, i64), DbErr> {
        let res = self
            ...
            .query_one(Statement::from_string(
                self.db.get_database_backend(),
                "SELECT server_count, online_count, total_players FROM global_stats WHERE id = 1"
                    .to_string(),
            ))
            .await?;

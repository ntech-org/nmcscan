...
pub async fn ping_server(addr: SocketAddr) -> Result<ServerStatus, Box<dyn std::error::Error + Send + Sync>> {
    ...
    let time = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH)?.as_millis() as u64;

...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        db.execute_unprepared(
            "CREATE INDEX IF NOT EXISTS idx_servers_players_max ON servers(players_max)",
        )
        .await?;
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        db.execute_unprepared("DROP INDEX IF EXISTS idx_servers_type_players_ip")
            .await?;
        db.execute_unprepared("DROP INDEX IF EXISTS trgm_idx_servers_brand")
            .await?;
        db.execute_unprepared("DROP INDEX IF EXISTS idx_servers_players_max")
            .await?;
